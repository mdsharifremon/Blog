        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form enctype="multipart/form-data" id="add-post-form">
                    ...
                    <div class="form__div">
                        <input type="text" name="title" class="form__input" placeholder=" " required>
                    ...
                    <div class="flex">
                        <div class="form-group md:w-6/12 w-ful pr-1 mb-3.5">
                            <select name="" id="category-select" class="form-control">
                                ...
                                <?php if ($row = $db->fetch_by_sql("SELECT * FROM taxonomy WHERE tax_type = 'category'")) : ?>
                                    <?php foreach ($row as $data) : ?>
                                        <option value="<?php echo $data['tax_id']; ?>"><?php echo $data['tax_name']; ?></option>
                                    <?php endforeach; ?>
                                <?php endif;  ?>
                            </select>
                        </div>
                        <div class="form-group md:w-6/12 w-full pl-1 mb-3.5">
                            <select name="Tags" id="tag-select" class="form-control">
                                <option disabled selected>Select Tags</option>
                                <?php if ($row = $db->fetch_by_sql("SELECT * FROM taxonomy WHERE tax_type = 'tag'")) : ?>
                                    <?php foreach ($row as $data) : ?>
                                        <option value="<?php echo $data['tax_id']; ?>"><?php echo $data['tax_name']; ?></option>
                                    <?php endforeach; ?>
                                <?php endif;  ?>
                    ...
                    <input type="submit" class="btn bg-blue-600 hover:bg-blue-700 text-gray-50 transition-0.3 w-full my-1" value="Post">
                </form>
            </div>
