...
if (isset($_POST['action']) && $_POST['action'] == 'postByTag') {
    ...
    if ($data = $db->fetch_by_sql($sql)) {
        foreach ($data as $row) :
            ...
            /** Read More Button */
            $output .= "<div><button class='bg-blue-500 hover:bg-blue-600 text-gray-100 text-sm rounded float-right mt-1 cursor-pointer view-post modal-open' data-target='#ViewPostModal' style='padding:0.14rem 0.5rem;' data-postId='{$row['post_id']}'>read more</button></div>";
