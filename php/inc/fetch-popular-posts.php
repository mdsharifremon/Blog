...
/**
 * @param Fetch_Popular_Posts
 ***********************/
if (isset($_POST['action']) && $_POST['action'] == 'fetchPopularPosts') {
    $sql = "SELECT p.post_id, p.post_title, p.post_image, p.post_created_on FROM posts p JOIN likes l ON p.post_id = l.likes_post ORDER BY likes_id DESC LIMIT 4";
    $output = '';
    if($data = $db->fetch_by_sql($sql)){
        foreach($data as $row):
            ...
            $output .= "<article>
            <a href='single-post.php?post_id={$row["post_id"]}' class='flex group modal-open' data-target='viewPostModal' data-postId='{$row['post_id']}'>";
        
           if($row['post_image'] != ''):
          $output .= "<figure class='flex-shrink-0'>
